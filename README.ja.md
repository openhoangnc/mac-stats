...
- 🔝 **リソースを食っているアプリが一目で分かる**: メニューを開くだけで、**CPU** と **メモリ** を最も消費しているアプリがリアルタイムでランキング表示されます。ヘルパープロセス（ブラウザの多数のレンダラーなど）は親アプリにまとめられ、バックグラウンドのシステムデーモンは除外されるため、本当に注目すべきアプリだけが表示されます。
- ⚙️ **便利な設定メニュー**: メニューバーのアイコンをクリック（または右クリック）すると、以下のメニューが表示されます：
  - **Open Activity Monitor（アクティビティモニタを開く）**: macOS標準のアクティビティモニタを開き、詳細な内訳を確認できます。
